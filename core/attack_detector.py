...
class AttackDetector:
    ...
    def _estimate_attack_type(self, text):
        ...
        # Using regex with word boundaries to avoid false positives (e.g., "door", "standard")
        # SQL Injection
        sql_pattern = r'\b(select|union|drop|insert|update|group by|order by)\b|--|\'|\"'
        if re.search(sql_pattern, text_lower) or re.search(r'\b(or|and)\b\s+\d=\d', text_lower):
            return 'SQL Injection'
        
        # XSS
        xss_pattern = r'<script|javascript:|onerror=|alert\(|<iframe|<svg|onload='
        if re.search(xss_pattern, text_lower):
            return 'XSS'
        
        # LFI / RFI
        lfi_pattern = r'\.\.\/|\.\.\\|\/etc\/passwd|php:\/\/|file:\/\/|proc\/self'
        if re.search(lfi_pattern, text_lower):
            return 'Local File Inclusion'
        
        # Brute Force
        brute_pattern = r'\b(login|admin|wp-login|password|auth|cmd)\b'
        if re.search(brute_pattern, text_lower):
