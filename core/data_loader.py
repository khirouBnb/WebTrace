...
def load_kaggle_data(datasets_path='datasets'):
    ...
    if os.path.exists(file1):
        ...
        try:
            ...
            req_col = 'Request' if 'Request' in df1.columns else 'Query' if 'Query' in df1.columns else None
            if req_col and 'Label' in df1.columns:
                all_requests.extend(df1[req_col].astype(str).tolist())
                # Vectorized label processing is much faster than iterrows
                all_labels.extend(df1['Label'].astype(str).str.contains('attack', case=False).astype(int).tolist())
    ...
    if os.path.exists(file2):
        ...
        try:
            ...
            if 'Query' in df2.columns and 'Label' in df2.columns:
                all_requests.extend(df2['Query'].astype(str).tolist())
                all_labels.extend(((df2['Label'].astype(str).str.contains('sql', case=False)) | (df2['Label'] == 1)).astype(int).tolist())
    ...
    if os.path.exists(file3):
        ...
        try:
            ...
            if 'Request' in df3.columns and 'Label' in df3.columns:
                all_requests.extend(df3['Request'].astype(str).tolist())
                all_labels.extend((df3['Label'] == 'attack').astype(int).tolist())
