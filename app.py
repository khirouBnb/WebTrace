...
def analyze_log():
    ...
    try:
        ...
        # Fallback for index.html compatibility
        access_file = request.files.get('log_file') or request.files.get('access_log_file')
        if not access_file or access_file.filename == '':
            return jsonify({'error': 'يرجى رفع ملف access.log'}), 400

        # قراءة محتوى ملف access.log
        ...
        if 'error_log_file' in request.files and request.files['error_log_file'].filename != '':
            ...
            error_log_info = f"تم رفع الملف ({error_file.filename})"
        ...
        case_info = {
            ...
            'access_log_filename': access_file.filename,
